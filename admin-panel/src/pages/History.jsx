...
const History = ({ user }) => {
    ...
    const groupedEvents = useMemo(() => {
        ...
        events.forEach(event => {
            const date = event.event_date; // Backend returns DD/MM/YYYY in event_date, might need parsing
            // Event date in backend: eventDate.toLocaleDateString('es-PE')
            if (!groups[date]) groups[date] = [];
                                ...
                                {!loading && activeTab === 'trips' && (
                                    <>
                                        {trips.length === 0 ? <div className="hx-empty-msg">📍 Sin recorridos en este rango</div> : (
                                            <div className="hx-history-container">
                                                {groupedTrips.map(group => (
                                                    <div key={group.date} className="hx-date-group">
                                                        <div className="hx-date-header">
                                                            <Calendar size={14} />
                                                            <span>{dayjs(group.date).format('dddd, D [de] MMMM [de] YYYY')}</span>
                                                        </div>
                                                        <div className="hx-trips-grid">
                                                            {group.trips.map(trip => <TripCard key={trip.id} trip={trip} onClick={() => handleTripClick(trip)} />)}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            <div className="hx-history-container">
                                                {groupedStops.map(group => (
                                                    <div key={group.date} className="hx-date-group">
                                                        ...
                                                        <div className="hx-stops-list">
                                                            {group.stops.map((stop, idx) => <StopRow key={idx} stop={stop} />)}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                {!loading && activeTab === 'events' && (
                                    <>
                                        {events.length === 0 ? <div className="hx-empty-msg">✨ Sin eventos</div> : (
                                            <div className="hx-history-container">
                                                {/* Events might need special date sorting if date is DD/MM/YYYY string */}
                                                {groupedEvents.map(group => (
                                                    <div key={group.date} className="hx-date-group">
                                                        ...
                                                        <div className="hx-events-list">
                                                            {group.events.map((event, idx) => <EventRow key={idx} event={event} />)}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
